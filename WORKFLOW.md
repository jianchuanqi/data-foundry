...
The queue state machine belongs to `npx --yes @tiangong-lca/cli@latest dataset curation-queue build/next/verify`. It writes task, lock, blocker, closure, input, run-plan, and queue status artifacts; it does not run AI or write the database.

After build, workers should call `npx --yes @tiangong-lca/cli@latest dataset curation-queue next --queue-dir .foundry/workspaces/<task-id>/curation-queue --json` and execute only the returned task. Before write planning, call `npx --yes @tiangong-lca/cli@latest dataset curation-queue verify --queue-dir .foundry/workspaces/<task-id>/curation-queue --type <support|flow|process> --json`.
